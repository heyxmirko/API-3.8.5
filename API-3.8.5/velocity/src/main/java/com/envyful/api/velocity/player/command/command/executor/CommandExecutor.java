package com.envyful.api.velocity.player.command.command.executor;
...
public class CommandExecutor {
    ...
    public CommandExecutor(String identifier, SenderType<?, ?> sender, int senderPosition, Object commandClass, Method executor,
                           ...
                           Pair<ArgumentInjector<?, CommandSource>, String>[] arguments,
    ...
    private int calculateRequiredArgs() {
        ...
        for (Pair<ArgumentInjector<?, CommandSource>, String> argument : this.arguments) {
     ...
     */
    public boolean canExecute(CommandSource sender) {
        ...
        return sender.hasPermission(this.requiredPermission);
    }
     ...
     */
    public boolean execute(CommandSource sender, String[] arguments) {
        Object[] args = new Object[this.arguments.length];
        int subtract = 0;
        int skipped = 0;

        for (int i = 0; i < this.arguments.length; i++) {
            Pair<ArgumentInjector<?, CommandSource>, String> argument = this.arguments[i];
        ...
        if (!((SenderType<CommandSource, ?>) this.sender).isAccepted(sender)) {
            return false;
        }

        args[this.senderPosition] = ((SenderType<CommandSource, ?>) this.sender).getInstance(sender);
